...
- **Conhecimento Remotion** → `references/remotion-official/` (doc oficial completa,
  ...
  (transitions, text-animations, audio, captions, effects, tailwind, calculate-metadata…).
- **Armadilhas** → `references/remotion-gotchas.md` (o que já custou tempo).
- **Assets pela studio** → as caixas de `scripts/models/` geram imagem/voz/música
  que entram no vídeo. (Ver "Integração" abaixo.)
- **Render headless** → `video/scripts/render.mjs`.
- **Inspiração (NÃO template)** → `video/examples/`: vídeos prontos de ideias
  recorrentes (ex: intro pra README). Se o pedido lembra um exemplo, use de
  referência — mas ainda construa sob medida.
...
2. **Crie uma pasta nova isolada** pro vídeo (ex: `./<nome>-video/`). Scaffold do
   projeto Remotion: `npx create-video@latest --yes --blank --no-tailwind .`
   (ou `--tailwind` se for usar; ver `remotion-official/rules/tailwind.md`).
3. **Planeje os assets ANTES** (mesma regra de imagem): liste tudo que o vídeo
   ...
   referências externas ANTES (peça links ao usuário, um por vez). Veja a regra
   de prompt no SKILL.md.
