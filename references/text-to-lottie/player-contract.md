...
> ⚠️ **NÃO se aplica ao studio.** Este arquivo descreve o player Skia **Skottie**
> da skill original (estrutura `public/projects/<project>/<scene>`, `?frame=N`,
> `/__context`, slots write-back). O studio **não usa Skottie** — o runtime é
> **lottie-web/Remotion**. Para o contrato real do studio (escrever JSON via
> builders, validar, salvar em `~/studio/<proj>/`, render via `lottie-box`), use
> `../lottie.md`. Mantido aqui só por completude da biblioteca copiada.
